...
// The shortcode
function srel_upcomming_events_shortcode() {


echo  '<div id="srel-main-div">'; 
echo '<div class="head-event"><h2>Upcoming Events</h2></div>';

	$paged = ( get_query_var( 'paged' ) ) ? get_query_var( 'paged' ) : 1; 
	$today = strtotime('today'); 

	$srel_meta_query = array( 
		'relation' => 'AND',
		array( 
			'key' => 'event-date', 
			'value' => $today, 
	...
	if ( $srel_events->have_posts() ) : 
		...
		echo  '<p>There are no upcomig events.</p>';
	endif; 
echo  '</div>';
} 

add_shortcode('list_upcoming_events', 'srel_upcomming_events_shortcode');
